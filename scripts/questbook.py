...
def parse_args():
    ...
    parser.add_argument("--prefix",
                        type=str,
                        default="tjceu",
                        help="prefix applied to questbook entries, typically the modpack id, defaults to tjceu")
...
def key(entry):
    ...
    return ("db" in entry, int(entry[15:(-5 if "desc" in entry else -6)]), "desc" in entry)
